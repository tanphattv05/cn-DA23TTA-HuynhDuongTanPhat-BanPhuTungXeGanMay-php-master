...
try {
    ...
    $total = (int) product_query('SELECT COUNT(*) AS total FROM products p' . $where, $types, $values)->get_result()->fetch_assoc()['total'];
    $pages = max(1, (int) ceil($total / 10));
    $page = min($page, $pages);
    $rows = product_query('SELECT p.*, c.name AS category_name FROM products p LEFT JOIN categories c ON c.id=p.category_id' . $where . ' ORDER BY p.id DESC LIMIT ? OFFSET ?', $types . 'ii', array_merge($values, [10, ($page - 1) * 10]))->get_result()->fetch_all(MYSQLI_ASSOC);
} catch (Throwable $e) {
