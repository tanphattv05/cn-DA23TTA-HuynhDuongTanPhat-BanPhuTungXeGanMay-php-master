...
if ($filterErrors) {
    ...
    try {
        [$where, $types, $values] = order_list_condition($filters);
        $total = (int) product_query('SELECT COUNT(*) AS total FROM orders' . $where, $types, $values)->get_result()->fetch_assoc()['total'];
        ...
        $orders = product_query(
            ...
            $types . 'ii', array_merge($values, [10, ($page - 1) * 10])
